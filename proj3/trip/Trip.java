...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        String startRoad, startDir, dest;
        int pVert, cVert;
        pVert = segment.get(1);
        Road start = _map.getLabel(from, pVert);
        startRoad = start.toString();
        startDir = start.direction().fullName();
        double startLength = start.length();
        Location target = _map.getLabel(segment.get(segment.size() - 1));
        dest = target.toString();
        Road intermediate = _map.getLabel(pVert, segment.get(2));
        Iterator<Integer> segIter = segment.subList(2,
                segment.size()).iterator();
        while (segIter.hasNext()) {
            cVert = segIter.next();
            intermediate = _map.getLabel(pVert, cVert);
            if (intermediate.toString().equals(startRoad)
                    && intermediate.direction().fullName().equals(startDir)) {
                startLength += intermediate.length();
            } else {
                System.out.printf("%d. Take %s %s for %.1f miles.%n", seq,
                        startRoad, startDir, startLength);
                startRoad = intermediate.toString();
                startDir = intermediate.direction().fullName();
                startLength = intermediate.length();
                seq += 1;
            }
            pVert = cVert;
        ...
        System.out.printf("%d. Take %s %s for %.1f miles to %s.%n",
                seq, intermediate, intermediate.direction().fullName(),
                startLength, dest);
        seq += 1;
        return seq;
    }
